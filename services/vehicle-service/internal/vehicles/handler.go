...
func (h *Handler) ListVehicles(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.db.Query(ctx, `
		SELECT vehicle_id, lat, lng, status, created_at
		FROM vehicles
	...
	for rows.Next() {
		var v Vehicle
		if err := rows.Scan(&v.VehicleID, &v.Lat, &v.Lng, &v.Status, &v.CreatedAt); err != nil {
...
func (h *Handler) CreateVehiclesBulk(w http.ResponseWriter, r *http.Request) {
	...
	for i := 0; i < req.Count; i++ {
		...
		tag, execErr := tx.Exec(ctx, `
			INSERT INTO vehicles (vehicle_id, lat, lng, status)
			VALUES ($1, $2, $3, 'active')
...
func (h *Handler) CreateVehicle(w http.ResponseWriter, r *http.Request) {
	...
	_, err := h.db.Exec(ctx, `
		INSERT INTO vehicles (vehicle_id, lat, lng, status)
		VALUES ($1, $2, $3, $4)
	`, req.VehicleID, req.Lat, req.Lng, req.Status)
	if err != nil {
		writeError(w, http.StatusConflict, "vehicle_id ya existe o no pudo crearse")
		return
	}

	writeJSON(w, http.StatusCreated, map[string]string{"vehicle_id": req.VehicleID, "status": "created"})
}
...
func (h *Handler) GetVehicle(w http.ResponseWriter, r *http.Request) {
	...
	err = h.db.QueryRow(ctx, `
		...
		FROM vehicles
		WHERE vehicle_id = $1
	`, vehicleID).Scan(&v.VehicleID, &v.Lat, &v.Lng, &v.Status, &v.CreatedAt)
	if err != nil {
...
func (h *Handler) StartSimulation(w http.ResponseWriter, r *http.Request) {
	...
	req := simulationStartRequest{SelectedCount: 50, TickMS: 1500}
	if r.Body != nil {
		if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
			writeError(w, http.StatusBadRequest, "payload invalido")
			return
