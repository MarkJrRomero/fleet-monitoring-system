...
export function AppShell({
  ...
  useEffect(() => {
    ...
    if (isFirstSidebarAnimRef.current) {
      ...
      gsap.set(labels, { opacity: isSidebarCollapsed ? 0 : 1 });
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 antialiased">
      ...
      <aside
        ref={sidebarRef}
        className={`fixed left-0 top-0 z-[1300] flex h-full w-72 -translate-x-full flex-col overflow-hidden border-r border-white/60 bg-white/90 p-4 shadow-xl backdrop-blur-xl transition-transform duration-300 lg:translate-x-0 ${desktopSidebarClass} ${isMobileSidebarOpen ? 'translate-x-0' : ''}`}
      >
        <div className={`${isSidebarCollapsed ? 'mb-2' : 'mb-8'} flex items-start justify-between gap-3`}>
          <div className="flex items-center gap-3">
            <div className={`sidebar-label overflow-hidden transition-[max-width,opacity] duration-300 ${isSidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-[180px] opacity-100'}`}>
        ...
        <div className={`sidebar-label overflow-hidden text-[10px] font-bold uppercase tracking-[0.22em] text-slate-400 transition-[max-width,opacity,margin] duration-300 ${isSidebarCollapsed ? 'mb-0 max-h-0 max-w-0 opacity-0' : 'mb-3 max-w-[180px] opacity-100'}`}>Navegacion</div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={
                  item.active
                    ? `flex w-full items-center rounded-xl bg-gradient-to-r from-teal-500 to-cyan-500 py-3 text-left text-white shadow-md shadow-teal-400/25 transition-colors ${isSidebarCollapsed ? 'justify-center px-0' : 'gap-3 px-3'}`
                    : `flex w-full items-center rounded-xl py-3 text-left text-slate-500 transition-colors hover:bg-slate-100/80 hover:text-slate-700 ${isSidebarCollapsed ? 'justify-center px-0' : 'gap-3 px-3'}`
                }
                to={item.to}
                onClick={() => setIsMobileSidebarOpen(false)}
              >
        ...
        <div className="mt-auto border-t border-slate-100 pt-3">
          <button
            className={`hidden w-full items-center rounded-xl border border-slate-200 bg-slate-50 py-2 text-sm font-medium text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-700 lg:flex ${isSidebarCollapsed ? 'justify-center px-0' : 'gap-2 px-3'}`}
            onClick={() => setIsSidebarCollapsed((prev) => !prev)}
            type="button"
          >
            {isSidebarCollapsed ? <PanelLeftOpen className="h-4 w-4" /> : <PanelLeftClose className="h-4 w-4" />}
            <span className={`sidebar-label overflow-hidden whitespace-nowrap transition-[max-width,opacity] duration-300 ${isSidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-[180px] opacity-100'}`}>{isSidebarCollapsed ? 'Expandir' : 'Colapsar sidebar'}</span>
          </button>
        </div>
      </aside>

      <header ref={headerElRef} className={`fixed right-0 top-0 z-[1200] flex h-16 w-full items-center justify-between border-b border-white/60 bg-white/80 px-4 shadow-sm backdrop-blur-xl sm:px-5 lg:px-8 ${desktopHeaderClass}`}>
        <div className="flex items-center gap-3">
          <button
